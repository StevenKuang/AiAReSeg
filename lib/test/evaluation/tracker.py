...
class Tracker:
    ...
    def run_sequence(self, seq, debug=None,segmentation=False):
        """
        ...
        output = self._track_sequence(tracker, seq, init_info, segmentation)

        return output

    def _track_sequence(self, tracker, seq, init_info, segmentation):
        """
        ...
        while out is True and index < len(seq.frames)-1:
            print(f"No catheter found, trying frame {index}!")
            image=[self._read_image(seq.frames[index])]
        ...
        if segmentation == True:
            init_default = {'target_mask': init_info.get('init_mask'),
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            if out is True:
                out = {}
                out['target_mask'] = [torch.zeros(size=(image[0].shape[0],image[0].shape[1]))]
        ...
        for key in ['target_bbox', 'all_boxes', 'all_scores', 'target_mask']:
